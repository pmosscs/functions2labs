...
// CODE HERE
function multiply(num1, num2, callfun) {
  return callfun(num1 * num2);
}
...
// CODE HERE 
const first = (arr, callFun) => {
  return callFun(arr[0]);
}
...
// CODE HERE
const last = (arr, callFun) => callFun(arr[arr.length - 1]);
...
// CODE HERE 
function contains(arr, name, callFun) {
  if (arr.includes(name)) {
    return callFun(true);
  } else {
    return callFun(false);
  }
}
...
// CODE HERE 
const getUserById = (user, id1, callFun) => {
  for (let i = 0; i < user.length; i++) {
    if (id1 === users[i].id) {
      return callFun(users[i]);
    } 
  }
